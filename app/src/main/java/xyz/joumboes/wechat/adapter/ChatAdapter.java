...
import xyz.joumboes.wechat.bean.MsgBody;
...
import xyz.joumboes.wechat.util.GlideUtils;
...
public class ChatAdapter extends BaseQuickAdapter<Message, BaseViewHolder> {
    ...
    private void setContent(BaseViewHolder helper, Message item) {
        if (item.getAvatar() != null) {
            if (bmpMap.get(item.getAvatar())!=null) {
                bmp = bmpMap.get(item.getAvatar());
            }else{
                bmp = BitmapFactory.decodeFile(item.getAvatar());
                bmpMap.put(item.getAvatar(),bmp);
            }
            helper.setImageBitmap(R.id.chat_item_header, bmp);
        }
